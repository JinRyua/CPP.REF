...
SString* Quaternion::ToString() const
{
	return SString::Format("{{Axis: {0}, Angle: {1}}}"_s, GetAxis(), GetAngle());
}
...
Vector3 Quaternion::RotateVector(const Vector3& v, bool bCoord) const
{
	XMVECTOR xmv = XMLoadVector3(&v);

	XMVECTOR Q = XMVectorSet(X, Y, Z, bCoord ? 1.0f : 0.0f);
...
void Quaternion::SetVectorPart(const Vector3& value)
{
	X = value.X;
	Y = value.Y;
	Z = value.Z;
...
Quaternion Quaternion::LookTo(const Vector3& forward, const Vector3& up)
{
	...
	Vector3 R = Vector3::CrossProduct(up, forward).GetNormalized();
