...
size_t Math::GetPrime(size_t min)
{
	size_t primes_length = _countof(Primes);
    ...
    for (size_t i = (min | 1); i < 0xFFFFFFFF; i += 2)
    {
...
size_t Math::ExpandPrime(size_t oldSize)
{
	...
	if (newSize > MaxPrimeArrayLength && MaxPrimeArrayLength > oldSize)
	{
		...
		{
			throw new SInvalidOperationException("Invalid MaxPrimeArrayLength"_s);
		}
...
bool Math::IsPrime(size_t candidate)
{
	if ((candidate & 1) != 0)
	{
		size_t limit = (size_t)Sqrt((float)candidate);
